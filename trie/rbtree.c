...
rb_node *rbtree_node_create(char key, void *value) {
...
rb_tree *rbtree_create() {
    ...
    tree->size = 0;
    tree->null = rbtree_node_create('\0', NULL);
...
rb_node *rbtree_insert(rb_tree *tree, char key, void *value) {
...
void rbtree_delete_node(rb_tree *tree, rb_node *root) {
    if (root == tree->null) return;

    rbtree_delete_node(tree, root->left);
    rbtree_delete_node(tree, root->right);
    free(root);
...
void rbtree_free(rb_tree *tree) {
    ...
    rbtree_delete_node(tree, tree->root);
    free(tree->null);
    free(tree);
}
...
void dfs_print_keys(rb_tree *tree, rb_node *root, int level) {
    if (!root || root == tree->null) return;

    dfs_print_keys(tree, root->left, level);
    printf("%c", root->key);
    if (root->key == '$') {
        printf("\n");
    }
    trie_nodes_print(root->value, level + 1);
    dfs_print_keys(tree, root->right, level);
}

void rbtree_print_keys(rb_tree *tree, int level) {
    if (!tree) return;
    
    dfs_print_keys(tree, tree->root, level);
}
